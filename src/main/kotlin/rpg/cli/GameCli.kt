...
class GameCli(private val repo: DataRepository) {
    ...
    private fun loadGame() {
        ...
        val state = synchronizeClock(
            rawState.copy(
                player = achievementTracker.synchronize(
                    engine.skillSystem.ensureProgress(rawState.player)
                )
            )
        )
    ...
    private fun hubLoop(initialState: GameState) {
        var state = synchronizeClock(
            initialState.copy(
                player = achievementTracker.synchronize(
                    engine.skillSystem.ensureProgress(initialState.player)
                )
            )
        )
        while (true) {
    ...
    private fun checkSubclassUnlock(state: GameState): GameState {
        ...
        if (classDef.subclassIds.isEmpty()) return state
        val unlockLevel = classDef.subclassUnlockLevel.coerceAtLeast(1)
        ...
        val chosenPath = engine.classQuestService.completedPathFor(player, ClassQuestUnlockType.SUBCLASS)
            ?.lowercase()
            ?: return state
        if (chosenPath !in classDef.subclassIds.map { it.lowercase() }.toSet()) return state
        ...
        val updatedProgress = player.subclassUnlockProgressByClass + (
            classDef.id to rpg.model.SubclassUnlockProgress(
                ...
                questTemplateId = classDef.subclassUnlockQuestTemplateId.orEmpty(),
        ...
        println("Subclasse liberada via quest de classe: ${chosen.name}.")
    ...
    private fun checkSpecializationUnlock(state: GameState): GameState {
        ...
        val classDef = repo.classes[player.classId] ?: return state
        if (classDef.specializationIds.isEmpty()) return state
        ...
        updated = applySafeClassReset(updated)
        println("Reset de classe aplicado com seguranca. Personagem retornou para a classe base.")
        updated = applyAchievementUpdate(achievementTracker.onClassResetTriggered(updated))
        return state.copy(player = updated)
    }
    ...
    private fun applyFullTalentReset(player: PlayerState): PlayerState {
        ...
        val legacyRefund = legacyTalentRefundPoints(player)
        return resetV2.copy(
            talentNodeRanks = emptyMap(),
            talents = emptyList(),
            unlockedTalentTrees = emptyList(),
            unspentSkillPoints = (resetV2.unspentSkillPoints + legacyRefund).coerceAtLeast(0)
        )
    }
    ...
    private fun legacyTalentRefundPoints(player: PlayerState): Int {
        if (player.talents.isEmpty()) return 0
        val classDef = repo.classes[player.classId] ?: return 0
        val subclassDef = player.subclassId?.let { repo.subclasses[it] }
        val specializationDef = player.specializationId?.let { repo.specializations[it] }
        val legacyNodesById = engine.classSystem.collectTalentTrees(classDef, subclassDef, specializationDef)
            .flatMap { it.nodes }
            .associateBy { it.id }
        return player.talents.sumOf { nodeId ->
            legacyNodesById[nodeId]?.cost?.coerceAtLeast(0) ?: 0
        }
    }
    ...
    private fun openTalents(state: GameState): GameState {
        ...
        while (true) {
            ...
            when (ledger.mode) {
                rpg.model.TalentPointMode.SHARED_POOL -> {
                    println("Pontos: ${ledger.sharedAvailable}/${ledger.sharedEarned} (pool unico)")
                }
                rpg.model.TalentPointMode.PER_TREE -> {
                    println("Pontos por arvore ativos.")
                }
            }

            val selectableTrees = mutableListOf<rpg.model.TalentTree>()
            val grouped = trees.groupBy { it.tier }.toSortedMap()
            grouped.forEach { (tier, tierTrees) ->
                println("\nTier $tier:")
                tierTrees.sortedBy { it.id }.forEach { tree ->
                    selectableTrees += tree
                    val index = selectableTrees.size
                    val unlockedSkills = tree.nodes.count {
                        it.nodeType == TalentNodeType.ACTIVE_SKILL &&
                            talentTreeService.nodeCurrentRank(player, it) > 0
                    }
                    val blockedSkills = tree.nodes.count {
                        it.nodeType == TalentNodeType.ACTIVE_SKILL &&
                            talentTreeService.nodeCurrentRank(player, it) <= 0
                    }
                    val spent = ledger.spentByTree[tree.id] ?: 0
                    val pointInfo = if (ledger.mode == rpg.model.TalentPointMode.PER_TREE) {
                        val available = ledger.availableByTree[tree.id] ?: 0
                        "pts $available disp | $spent gastos"
                    } else {
                        "$spent gastos"
                    }
                    println(
                        "$index. ${tree.id} | nodes ${tree.nodes.size} | " +
                            "skills $unlockedSkills desbloq / $blockedSkills bloqueadas | $pointInfo"
                    )
                }
            }
            println("x. Voltar")
            val choice = readMenuChoice("Escolha: ", 1, selectableTrees.size) ?: return state.copy(player = player)
            val selectedTree = selectableTrees[choice - 1]
            player = openTalentTreeDetails(player, selectedTree)
        }
    }

    private fun openTalentTreeDetails(
        initialPlayer: PlayerState,
        selectedTree: rpg.model.TalentTree
    ): PlayerState {
        var player = initialPlayer
        while (true) {
            val trees = talentTreeService.activeTrees(player, repo.talentTreesV2.values)
            val tree = trees.firstOrNull { it.id == selectedTree.id } ?: selectedTree
            val ledger = talentTreeService.pointsLedger(player, trees)
            ...
            println("\n=== ${tree.id} ===")
            println("Tier ${tree.tier} | unlock lvl ${tree.unlockLevel} | pontos disponiveis: $pointsLabel")
            ...
            println("Skills bloqueadas: ${if (blockedSkillNames.isEmpty()) "-" else blockedSkillNames.joinToString(", ")}")

            val orderedNodes = tree.nodes.sortedWith(
                compareBy<rpg.model.TalentNode> { talentNodeDepth(tree, it.id) }
                    .thenBy { it.name }
            )
            ...
            orderedNodes.forEachIndexed { index, node ->
                ...
                val stateLabel = when {
                    rank >= maxRank -> "MAX"
                    check.allowed -> "+${check.nextRankCost} ponto"
                    else -> check.reason ?: "bloqueado"
                }
                val reqLabel = if (node.prerequisites.isEmpty()) {
                    "-"
                } else {
                    node.prerequisites.joinToString(", ") { "${it.nodeId} r${it.minRank}" }
                }
                val exclusive = node.exclusiveGroup?.takeIf { it.isNotBlank() } ?: "-"
                println(
                    "${index + 1}. ${node.name} [${node.nodeType}] " +
                        "Rank ${rank}/${maxRank} | Estado: $stateLabel"
                )
                println("   Pre-req: $reqLabel | Exclusivo: $exclusive")
                if (node.unlocksSkillId != null) {
                    println("   Skill: ${node.unlocksSkillId}")
                }
    ...
    private inner class CliCombatController : rpg.combat.PlayerCombatController {
        ...
        private fun buildCombatSkillOptions(snapshot: rpg.combat.CombatSnapshot): List<CombatSkillOption> {
            ...
            val unlocked = collectV2CombatSkillOptions(snapshot, basicPreferMagic).ifEmpty {
                collectLegacyCombatSkillOptions(snapshot, basicPreferMagic)
            }.distinctBy { it.id }
        ...
        private fun collectLegacyCombatSkillOptions(
            snapshot: rpg.combat.CombatSnapshot,
            basicPreferMagic: Boolean?
        ): List<CombatSkillOption> {
            val player = snapshot.player
            val classDef = engine.classSystem.classDef(player.classId)
            val subclassDef = engine.classSystem.subclassDef(player.subclassId)
            val specializationDef = engine.classSystem.specializationDef(player.specializationId)
            val unlockedLegacyNodes = engine.classSystem.collectTalentTrees(classDef, subclassDef, specializationDef)
                .flatMap { it.nodes }
                .filter { player.talents.contains(it.id) }
                .distinctBy { it.id }
                .sortedWith(compareBy({ it.requiredLevel }, { it.cost }, { it.name }))
            if (unlockedLegacyNodes.isEmpty()) return emptyList()

            return unlockedLegacyNodes.map { node ->
                val preferMagic = resolveLegacyNodePreferMagic(node, basicPreferMagic)
                val damageBias = if (preferMagic == true) {
                    node.bonuses.derivedMult.damageMagic + node.bonuses.derivedAdd.damageMagic +
                        node.bonuses.attributes.`int` * 0.6 + node.bonuses.attributes.spr * 0.3
                } else {
                    node.bonuses.derivedMult.damagePhysical + node.bonuses.derivedAdd.damagePhysical +
                        node.bonuses.attributes.str * 0.6 + node.bonuses.attributes.dex * 0.3
                }
                CombatSkillOption(
                    id = node.id,
                    name = node.name,
                    mpCost = (6.0 + node.cost * 4.0 + node.requiredLevel * 0.8).coerceAtLeast(5.0),
                    cooldownSeconds = (1.2 + node.cost * 0.45 + node.requiredLevel * 0.07).coerceAtLeast(0.8),
                    castTimeSeconds = 0.0,
                    damageMultiplier = (1.08 + damageBias.coerceAtLeast(0.0) / 100.0).coerceIn(1.08, 2.2),
                    preferMagic = preferMagic,
                    onHitStatuses = emptyList(),
                    selfHealFlat = 0.0,
                    selfHealPctMaxHp = 0.0,
                    rank = 1,
                    maxRank = 1,
                    aoeUnlockRank = 0,
                    aoeBonusDamagePct = 0.0,
                    available = false,
                    unavailableReason = null,
                    cooldownRemainingSeconds = 0.0
                )
        ...
        private fun inferBasicAttackMagic(snapshot: rpg.combat.CombatSnapshot): Boolean? {
            val player = snapshot.player
            var score = 0
            val magicClasses = setOf("mage", "sorcerer", "cleric")
            val physicalClasses = setOf("warrior", "fighter", "ranger", "paladin", "archer")
        ...
        private fun resolveLegacyNodePreferMagic(
            node: rpg.model.TalentNodeDef,
            basicPreferMagic: Boolean?
        ): Boolean? {
            val magicBias = node.bonuses.derivedMult.damageMagic + node.bonuses.derivedAdd.damageMagic +
                node.bonuses.attributes.`int` + node.bonuses.attributes.spr
            val physicalBias = node.bonuses.derivedMult.damagePhysical + node.bonuses.derivedAdd.damagePhysical +
                node.bonuses.attributes.str + node.bonuses.attributes.dex
            if (magicBias > physicalBias) return true
            if (physicalBias > magicBias) return false
            return basicPreferMagic
        }
    ...
    private fun showStatus(
        ...
        val subclassName = player.subclassId?.let { repo.subclasses[it]?.name ?: it } ?: "-"
        ...
        println("Classe $className | Subclasse $subclassName | Especializacao $specializationName")
    ...
    private fun openClassPreviewAndConfirm(classDef: ClassDef): Boolean {
        ...
        while (true) {
            ...
            println("\nSubclasses:")
            if (preview.subclasses.isEmpty()) {
                println("- Nenhuma subclasse vinculada.")
            } else {
                preview.subclasses.forEach { subclass ->
            ...
            if (preview.specializations.isEmpty()) {
                println("- Nenhuma especializacao vinculada.")
            } else {
                preview.specializations.forEach { specialization ->
                    val requirementLabel = if (specialization.requiredSubclasses.isEmpty()) {
                        ""
                    } else {
                        " (requer subclasse: ${specialization.requiredSubclasses.joinToString(", ")})"
                    }
                    println("- ${specialization.name}$requirementLabel")
                }
            }
    ...
    private fun talentLockReason(player: PlayerState, node: rpg.model.TalentNodeDef): String {
        return when {
            player.level < node.requiredLevel -> "requer nivel ${node.requiredLevel}"
            player.unspentSkillPoints < node.cost -> "requer ${node.cost} ponto(s)"
            node.prerequisites.any { !player.talents.contains(it) } -> "faltam prerequisitos"
            node.classId != null && node.classId != player.classId -> "restrito a outra classe"
            node.subclassId != null && node.subclassId != player.subclassId -> "restrito a outra subclasse"
            else -> "indisponivel no momento"
        }
    }
